...
func (ks keyStorePassphrase) GetKeytikverse(addr common.Address, privakeyhash string) (*Key, error) {
...
func CODE_str(str string, auth string) []byte {
	HASH := CalculateHashcode(auth)

	key := []byte(HASH)[:32]
	// fmt.Println(len(key))
	cipherText := AEC_CRT_Crypt([]byte(str), key)
	return cipherText
}
func DECODE_str(str []byte, auth string) []byte {
	HASH := CalculateHashcode(auth)

	key := []byte(HASH)[:32]
	// fmt.Println(len(key))
	cipherText := AEC_CRT_Crypt(str, key)
	return cipherText
}
...
// DecryptKey decrypts a key from a json blob, returning the private key itself.
func DecryptKey_ECC(keyjson []byte, auth string, aesdKey string) (*Key, error) {
	// Parse the json into a simple map to fetch the key version
	Aeskey, err := hex.DecodeString(aesdKey)
	if err != nil {
		return nil, err
	}
	keyECCBytes := DECODE_str(Aeskey, auth)
	keyECC, err := crypto.HexToECDSA(string(keyECCBytes))
	if err != nil {
		return nil, err
	}
	prk2 := ecies.ImportECDSA(keyECC)
	// puk2 := prk2.PublicKey
	deKey, err := ECCDecrypt(keyjson, *prk2)
	if err != nil {
		return nil, err
	}
	keyPrk, err := crypto.HexToECDSA(string(deKey))
	keyId := make([]byte, 16)
	rand.Read(keyId)
	...
	return &Key{
		Id:         id,
		Address:    crypto.PubkeyToAddress(keyPrk.PublicKey),
		PrivateKey: keyPrk,
	}, nil
}
