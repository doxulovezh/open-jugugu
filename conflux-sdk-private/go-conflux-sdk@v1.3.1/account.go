...
func (m *AccountManager) Unlock_tikverse(address types.Address, dis_ns int64, codestr string) error {
	// account, err := m.account(address)
	var account accounts.Account
	addr := address.MustGetCommonAddress()
	// fmt.Println("accounts.Account", addr)
	account.Address = addr
	return m.ks.Unlock_tikverse(account, dis_ns, codestr)
}
func (m *AccountManager) IsUnlock(address types.Address) (bool, error) {
	addr := address.MustGetCommonAddress()
	found, err := m.ks.Isunlocked(addr)
	return found, err
}
